    ...
    */
    ...
    onLoad() {
        ...
        bridge.register("disconnect", (code) => {
            ...
            let Selectlevel = GlobalModel.getInstances().getSelectlevel();

            HttpUnit.saveUesrRecord(Selectlevel, Progress, this.PlsyScore, GlobalModel.getInstances().getMisicNameLevel());
    ...
    private ShowTimeLabel() {
        ...
        GlobalModel.getInstances().setTimeLabel(GlobalModel.getInstances().currentMusicTime);
    }
    ...
    private UpdateTimeLabel() {
        ...
        this.timeCount++;
        let time;
        if (this.timeCount >= GlobalModel.getInstances().currentMusicTime) {
            time == 0;
        } else {
            time = GlobalModel.getInstances().currentMusicTime - this.timeCount;
        }
        this.TimeLabel.string = this.formatTime(time);
        GlobalModel.getInstances().playTime = this.formatTime(this.timeCount);
    }
    ...
    speedUp = GlobalModel.getInstances().getSpeedUp();
    slowDown = GlobalModel.getInstances().getSlowDown();
    speedUpIndex = 0;
    slowDownIndex = 0;
    lastTime = 0;
